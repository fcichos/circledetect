...
def compute_velocities(trajectory, fps, smoothing_window=5):
    ...
    for i, frame in enumerate(frames):
        if i == 0:
            ...
            x_next, y_next, _ = trajectory[frames[i]]
...
def transform_video_to_particle_frame(
    ...
    Args:
        ...
        velocity_axis: Target axis for velocity alignment ('horizontal' or 'vertical')
        reference_direction: Direction for velocity ('right', 'left', 'up', 'down')
    ...
    output_video_path = get_video_path(f"{basename}_particle_frame.mp4")
    params_csv_path = get_csv_path(f"{basename}_particle_frame_params.csv")

    if annotate:
        annotated_video_path = get_video_path(f"{basename}_particle_frame_annotated.mp4")
...
def main():
    parser = argparse.ArgumentParser(
        description='Transform video to particle reference frame',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage
  python scripts/transform_to_particle_frame.py data/video.mp4 results/csv/video_trajectory_stabilized_filtered.csv

  # With options
  python scripts/transform_to_particle_frame.py data/video.mp4 results/csv/video_trajectory_subpixel.csv --smoothing-window 7 --annotate

  # Without rotation (just center particle - avoids jitter from Brownian motion)
  python scripts/transform_to_particle_frame.py data/video.mp4 results/csv/video_trajectory_stabilized_filtered.csv --no-rotation
        """
    )

    parser.add_argument('video_path', help='Path to input video file')
    parser.add_argument('trajectory_csv', help='Path to trajectory CSV file')
    parser.add_argument('--smoothing-window', type=int, default=5,
                       help='Velocity smoothing window size (default: 5)')
    parser.add_argument('--velocity-axis', default='horizontal',
                       choices=['horizontal', 'vertical'],
                       help='Target axis for velocity alignment (default: horizontal)')
    parser.add_argument('--reference-direction', default='right',
                       choices=['right', 'left', 'up', 'down'],
                       help='Direction for velocity vector (default: right)')
    parser.add_argument('--annotate', action='store_true',
                       help='Create annotated video with axes and info')
    parser.add_argument('--no-rotation', action='store_true',
                       help='Disable rotation - only center particle (avoids jitter from Brownian motion)')
    ...
    transform_video_to_particle_frame(
        args.video_path,
        args.trajectory_csv,
        velocity_smoothing_window=args.smoothing_window,
        velocity_axis=args.velocity_axis,
